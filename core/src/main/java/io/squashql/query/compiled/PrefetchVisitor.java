...
public class PrefetchVisitor implements MeasureVisitor<Map<QueryScope, Set<CompiledMeasure>>> {
  ...
  @Override
  public Map<QueryScope, Set<CompiledMeasure>> visit(CompiledVectorTupleAggMeasure compiledVectorTupleAggMeasure) {
    /*
     * The trick here is to generate a query with a subquery that first compute the aggregate of measure.fieldToAggregate
     * with the aggregation function measure.aggregationFunction + the correct rollup and then the top query computes
     * the final aggregates to create the vector. To do that, we use aliases and grouping functions.
    ...
    List<TypedField> fieldToAggregates = compiledVectorTupleAggMeasure.fieldToAggregateAndAggFunc().stream().map(Pair::getOne).toList();
    List<String> vectorAggFuncs = compiledVectorTupleAggMeasure.fieldToAggregateAndAggFunc().stream().map(Pair::getTwo).toList();
    if (this.originalQueryScope.columns().contains(vectorAxis)) {
      Set<CompiledMeasure> m = new HashSet<>();
      for (int i = 0; i < fieldToAggregates.size(); i++) {
        m.add(new CompiledAggregatedMeasure(compiledVectorTupleAggMeasure.alias(), fieldToAggregates.get(i), vectorAggFuncs.get(i), null, false));
      }
      ...
      for (int i = 0; i < subQueryMeasureAliases.size(); i++) {
        topQueryMeasures.add(new CompiledAggregatedMeasure(compiledVectorTupleAggMeasure.alias() + "_" + i, new AliasedTypedField(subQueryMeasureAliases.get(i)), ARRAY_AGG, null, false));
      }
      return Map.of(topQueryScope, topQueryMeasures);
    }
  }
