...
public class SQLTranslator {
  ...
  private static Map<String, ConditionDto> extractConditions(DatabaseQuery query) {
    ...
    query.coordinates.forEach((field, values) -> {
      if (values != null && values.size() == 1) {
        ...
        conditionByField.put(field, new MultipleValueConditionDto(values));
      }
    });
  ...
  public static String toSql(Field field, ConditionDto dto) {
    if (dto instanceof SingleValueConditionDto || dto instanceof MultipleValueConditionDto) {
      ...
      return switch (dto.type()) {
        case IN -> escape + " in (" +
                ((MultipleValueConditionDto) dto).values
                        .stream()
                        .map(sqlMapper)
                        .collect(Collectors.joining(", ")) + ")";
