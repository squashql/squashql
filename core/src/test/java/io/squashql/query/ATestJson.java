...
//    this.executor.executeRaw("SELECT JSON_VALUE(JSON '{\"name\": \"Jakob\", \"age\": \"6\" }', '$.age') from " + this.executor.queryEngine.queryRewriter().tableName(this.storeName));
    this.executor.executeRaw("select price_construction_array::json->>1 from " + this.executor.queryEngine.queryRewriter().tableName(this.storeName)).show();
    Assertions.assertThat(table).containsExactly(
            List.of("A", "{\"a\":true,\"b\":\"idA\",\"c\":0}"),
            List.of("B", "{\"a\":true,\"b\":\"idB\",\"c\":1}"));
  }
}
