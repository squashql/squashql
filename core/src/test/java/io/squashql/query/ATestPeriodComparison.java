...
public abstract class ATestPeriodComparison extends ABaseTestQuery {
  ...
  private final TableField year = new TableField(this.storeName, "year_sales"); // Use long to make sure we support also long as type
  ...
  void testCompareYearCurrentWithPrevious() {
    ...
    var query = Query.from(this.storeName)
            .select(tableFields(List.of("year_sales")), List.of(m, sales))
            .build();
    Table finalTable = this.executor.executeQuery(query);
    Assertions.assertThat(finalTable).containsExactlyInAnyOrder(
    ...
    query = Query.from(this.storeName)
            .select(tableFields(List.of("year_sales")), List.of(m, sales))
            .rollup(tableFields(List.of("year_sales")))
            .build();
    finalTable = this.executor.executeQuery(query);
    Assertions.assertThat(finalTable).containsExactlyInAnyOrder(
  ...
  void testCompareYearCurrentWithPreviousFullName() {
    ...
    AggregatedMeasure sales = new AggregatedMeasure("sum(sales)", this.sales, "sum", null);
    ComparisonMeasureReferencePosition m = new ComparisonMeasureReferencePosition(
            "myMeasure",
            ABSOLUTE_DIFFERENCE,
            sales,
  ...
  void testCompareSemesterCurrentWithPrevious() {
    ...
    var query = Query.from(this.storeName)
            .select(tableFields(List.of("year_sales", "semester_sales")), List.of(m, sales))
            .build();
  ...
  void testCompareMonthCurrentWithPrevious() {
    ...
    var query = Query.from(this.storeName)
            ...
            .select(tableFields(List.of("year_sales", "month_sales")), List.of(m, sales))
            .build();
  ...
  void testPeriodIsMissingFromQuery() {
    ...
    var query = Query.from(this.storeName)
            .select(tableFields(List.of("ean")), List.of(m))
            .build();
  ...
  void testCompareYearCurrentWithPreviousWithFilterAndCalculatedMeasure() {
    ...
    var query = Query.from(this.storeName)
            ...
            .select(tableFields(List.of("year_sales")), List.of(m, multiply))
            .build();
    Table finalTable = this.executor.executeQuery(query);
    Assertions.assertThat(finalTable).containsExactlyInAnyOrder(
