...
Condition operators available: `eq, neq, lt, le, gt, ge, _in, notIn, isNull, isNotNull, like, and, or, not`.
...
An aggregate measure is computed by applying an aggregation function over a list of field values such as `sum`, `min`, `max`, `avg`, `count`, `countDistinct`, `anyValue`...
